...
export default function CartDrawer({
  ...
  return (
    <>
      ...
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-brand-yellow/20">
          <div>
        ...
        <div className="flex-1 overflow-y-auto px-5">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-slate-300">
              ...
              <p className="text-sm font-semibold text-slate-400">{t("cart.empty", locale)}</p>
              <p className="text-xs mt-1 text-slate-300">{t("cart.emptySub", locale)}</p>
            </div>
          ) : (
            <div className="py-2">
              {items.map((item) => (
        ...
        {/* Footer */}
        {items.length > 0 && (
          <div className="border-t border-slate-100 p-5 space-y-3 bg-gradient-to-t from-brand-yellow/5 to-transparent">
            ...
            <button
              onClick={clearCart}
              className="w-full py-2 text-xs text-slate-400 hover:text-red-500 transition-colors font-medium"
            >
