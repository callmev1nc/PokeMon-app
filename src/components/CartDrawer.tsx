    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-amber-500/10">
          ...
          <button
            onClick={onClose}
            className="p-2 rounded-xl hover:bg-slate-800 text-slate-500 hover:text-amber-400 transition-colors"
            aria-label="Close"
          >
        ...
        <div className="flex-1 overflow-y-auto px-5">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-slate-600">
              ...
              <p className="text-xs mt-1 text-slate-600">{t("cart.emptySub", locale)}</p>
            </div>
          ) : (
            <div className="py-2">
              {items.map((item) => (
        ...
        {/* Footer */}
        {items.length > 0 && (
          <div className="border-t border-amber-500/10 p-5 space-y-3 bg-[#0A0F1E]">
            ...
            <button
              onClick={clearCart}
              className="w-full py-2 text-xs text-slate-600 hover:text-red-400 transition-colors font-medium"
            >
