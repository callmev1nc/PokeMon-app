    ...
    return (
      <>
        ...
        <div className="max-w-4xl mx-auto px-4 py-16 text-center">
          ...
          <p className="text-lg font-semibold text-slate-400">{t("product.notFound", locale)}</p>
          <a href="/" className="text-brand text-sm mt-3 inline-flex items-center gap-1 hover:underline font-medium">
  ...
  return (
    <>
      ...
      <main className="max-w-5xl mx-auto px-4 py-6 animate-fade-in">
        {/* Breadcrumb */}
        <a href="/" className="text-xs text-slate-400 hover:text-brand transition-colors mb-5 inline-flex items-center gap-1 font-medium">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
          <div className="md:flex">
            {/* Image */}
            <div className="md:w-2/5 p-6 md:p-8 flex items-center justify-center bg-gradient-to-br from-slate-50 to-white relative">
              <div className="w-full max-w-[300px] relative">
                ...
                />
                {/* Subtle card frame glow */}
                <div className="absolute -inset-3 bg-gradient-to-br from-brand-yellow/10 via-transparent to-brand/10 rounded-2xl -z-10 blur-sm" />
              </div>
            </div>
            ...
            <div className="md:w-3/5 p-6 md:p-8 flex flex-col gap-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h1 className="text-2xl md:text-3xl font-bold text-slate-900 leading-tight" style={{ fontFamily: "var(--font-display)" }}>
                    {product.name}
                  </h1>
                  {product.series && (
                    <p className="text-sm text-slate-400 mt-1 font-mono tracking-tight">{product.series}</p>
                  )}
                </div>
                <button
                  ...
                  className={`p-2.5 rounded-xl border transition-all duration-300 flex-shrink-0 ${
                    isWished
                      ...
                      : "border-slate-200 text-slate-300 hover:text-red-400 hover:border-red-200"
                  }`}
              ...
              <div className="flex flex-wrap items-center gap-2">
                <span className={`px-3 py-1 text-[10px] font-bold rounded-md uppercase tracking-wide ${TYPE_COLORS[product.displayType] || "bg-slate-100 text-slate-600"}`}>
                  {product.displayType}
                </span>
                <span className="px-3 py-1 text-[10px] font-bold rounded-md uppercase tracking-wide bg-slate-100 text-slate-600">
              ...
              {/* Price */}
              <div className="py-5 border-t border-b border-slate-100">
                <p className={`text-3xl md:text-4xl font-bold tracking-tight ${noPrice ? "text-slate-300" : "text-slate-900"}`} style={{ fontFamily: "var(--font-display)" }}>
                  {formatPrice(product.price)}
                </p>
                <p className="text-sm text-slate-400 mt-1.5">
                  {t("product.stock", locale)}{" "}
                  <span className={`font-semibold ${product.stock <= 3 ? "text-orange-500" : "text-slate-600"}`}>
                    {product.stock}
                  </span>
                  {inCart > 0 && (
              ...
              <p className="text-xs text-slate-400">
                ...
                <span className="font-mono font-bold text-slate-500 bg-slate-50 px-2 py-0.5 rounded">{product.code}</span>
              </p>
                ...
                <div className="flex items-center gap-3 mt-auto pt-2">
                  <div className="flex items-center bg-slate-50 rounded-xl overflow-hidden border border-slate-100">
                    <button
                      ...
                      className="px-3.5 py-2.5 text-slate-500 hover:text-slate-700 hover:bg-slate-100 font-bold transition-colors"
                    >
                      -
                    </button>
                    <span className="px-3 py-2.5 text-sm font-bold min-w-[40px] text-center text-slate-700">
                      {qty}
                    </span>
                    <button
              ...
              {isOutOfStock && (
                <button disabled className="w-full py-3.5 rounded-xl text-sm font-semibold bg-slate-50 text-slate-300 cursor-not-allowed mt-auto">
        ...
        {related.length > 0 && (
          <section className="mt-12">
            <h2 className="text-lg font-bold text-slate-800 mb-5" style={{ fontFamily: "var(--font-display)" }}>
