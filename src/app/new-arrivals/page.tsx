...
export default function NewArrivalsPage() {
  ...
  useEffect(() => {
    fetch("/api/products")
      .then((res) => res.json())
      .then((data) => {
        const all: Product[] = Array.isArray(data) ? data : data?.data || [];
        setProducts([...all].reverse().slice(0, 100));
        setLoading(false);
      })
  ...
  return (
    <>
      ...
      <main className="flex-1 bg-[#FAFAFA] dark:bg-[#0F1629]">
        <section className="bg-gradient-to-r from-[#F6AD55] to-[#E53E3E] py-10 text-center">
          <h1 className="text-white text-4xl font-black tracking-wide" style={{ fontFamily: "var(--font-display)" }}>
            ✨ New Arrivals
          </h1>
          <p className="text-white/80 text-sm mt-2">Recently added cards to our collection</p>
        </section>
