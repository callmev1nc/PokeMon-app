  ...
  useEffect(() => {
    fetch("/api/products")
      ...
      .then((data) => {
        ...
        setProducts(all.filter((p) => p.stock > 0 && p.stock <= 10 && p.price !== null).sort((a, b) => a.stock - b.stock));
        setLoading(false);
      })
    ...
    <>
      ...
      <main className="flex-1 bg-[#FAFAFA] dark:bg-[#0F1629]">
        <section className="bg-gradient-to-r from-[#E53E3E] to-[#FC8181] py-10 text-center">
          ...
          <p className="text-white/80 text-sm mt-2">Low stock items — grab them before they&apos;re gone!</p>
        </section>
