...
function addProductToMenu(product) {
  // Add product to the Stock sheet (Tồn Kho t3) since that's the source of truth
  var stockSS = SpreadsheetApp.openById(STOCK_SS_ID);
  var sheet = getSheet(stockSS, "Tồn Kho t3");
  if (!sheet) return { error: "Stock sheet not found" };
  ...
  sheet.getRange(nextRow, 1).setValue(product.code || "");        // Code
  sheet.getRange(nextRow, 2).setValue(product.group || "");        // Group
  sheet.getRange(nextRow, 3).setValue(product.name || "");         // Name
